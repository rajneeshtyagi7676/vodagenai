...
Scenario: TMF629 Customer Management - resource /referenceDataList/{listName} for response code 200 OK
  Given with the server api.test.asgw.sol-vf.de for api /customerManagement/v1 and the resource /referenceDataList
  ...
  And each ReferenceDataList object should have the optional properties: active, default, fullLabelDisplay, href, selectionDisplay, shortLabelDisplay, extensions, name, referenceDataList