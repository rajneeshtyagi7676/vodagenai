...
Scenario: TMF629 Customer Management - resource /customer/{customerId}/addRelatedParty for response code 200 OK
  ...
  And the Customer object should have the optional properties: name, status, statusDate, statusReason, account, characteristic, contactMedium, creditProfile, description, engagedParty, href, paymentMethod, relatedParty, validFor, customerType, customerSubType, rank, acquisitionDate, agreement, createdOn, lastModifiedOn, extensions
  And the Customer object relatedParty should contain the newly added party