...
Scenario: TMF629 Customer Management - resource /creditProfile/{creditProfileId} for response code 200 OK
  ...
  And each CreditProfileCharacteristic in the characteristic array should have the properties: id, name, value, valueType, category, overridden
  And each PartyCreditRating in the creditRating array should have the properties: id, ratingScore, creditRiskRating, ratingAgencyName, ratingAgencyType, ratingReference, primary, validFor, characteristic, extensions
  And each ExternalIdentifier in the externalIdentifier array should have the properties: id, owner, externalIdentifierType, version, extensions