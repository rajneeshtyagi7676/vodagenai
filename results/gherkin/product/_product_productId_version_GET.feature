...
Scenario: TMF637 Product Inventory Management - resource /product/{productId}/version for response code 200 OK
  ...
  And each Product object should have the optional properties: id, href, name, description, isBundle, isCustomerVisible, productOffering, productSpecification, productCharacteristic, productOfferingCharacteristic, status, startDate, terminationDate, activationDate, orderDate, version, productPrice, productTotalPrice, productRelationship, relatedParty, place, billingAccount, productPromotion, productTerm, businessActivity, pathToRoot, parentId, rootProductId, primaryProductId, productSerialNumber, quantity, message, characteristicMetadata, extensions, @baseType, @type